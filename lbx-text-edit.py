...
Usage examples:
- Regular find/replace: lbx-text-edit.py replace input.lbx -f "2x2" -r "2×2"
- Regex find/replace: lbx-text-edit.py replace input.lbx -f "(\d+)x(\d+)" -r "\1×\2" --regex
  This example converts "2x4" to "2×4" by capturing digit groups and using the multiplication symbol
- Regex to add suffix: lbx-text-edit.py replace input.lbx -f "^(.*)$" -r "\1 Brick" --regex
  This adds " Brick" to the end of each text object
- Regex to format part numbers: lbx-text-edit.py replace input.lbx -f "(\d{4})(\d)?" -r "Part #\1-\2" --regex
