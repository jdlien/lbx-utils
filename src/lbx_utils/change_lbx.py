...
"""
change_lbx.py - Brother P-touch LBX Label File Modifier

This script modifies Brother P-touch LBX files to change label width, font size, and other properties.
It can also add a center cut line and apply various text transformations.

Basic Usage:
python3 change_lbx.py input.lbx output.lbx -f 14 -l 24 -c -s 1.5 -m 0.5

For help:
python3 change_lbx.py --help
...
class Config:
    ...
    def __init__(self):
        ...
        self.PTOUCH_EDITOR_APP_PATH: str = "/Applications/Brother P-Touch Editor.app"
...
def modify_lbx(lbx_file_path: str, output_file_path: str, options: Dict[str, Any]) -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if options.get('open_file', False) and platform.system() == 'Darwin':
            try:
                subprocess.run(['open', '-a', config.PTOUCH_EDITOR_APP_PATH, output_file_path])
